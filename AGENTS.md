...
- **soliplex-dev** — one-shot dev task runner, held out of `docker compose up` by `profiles: ["devmode"]`. Built from the backend image and mirroring its secrets, mounts and environment — the same `/app/src` paths and, via a YAML alias, the very same `PYTHONPATH`, so `soliplex-cli audit` here validates the installation the backend actually serves. The one difference is that `src/` is mounted **read-write**, for tools that write inside a project directory: `docker compose run --rm -w /app/src/<project> soliplex-dev /app/.venv/bin/alembic …` (alembic writes into its `versions/` directory and prepends `.` to `sys.path`). Any project directory under `src/` works, not just a `soliplex` checkout.
...
- **haiku-rag** — one-shot runner for the `haiku-rag` CLI, held out of `docker compose up` by `profiles: ["tools"]` (`docker compose run` enables the profile itself). Shares the ingester's image (via the `*haiku_rag_image` alias), its `rag/db` + `rag/docs` mounts, its environment and its `ingester_db_password` secret; everything after the service name is passed to the CLI: `docker compose run --rm haiku-rag info --db /data/haiku.rag.lancedb`. Its entrypoint exports `INGESTER_DB_PASSWORD` from the secret first, which every CLI call needs — see "Ingester control plane auth" below. Reads are safe while the stack is up; writing a store the ingester owns needs `docker compose stop haiku-ingester` first.
...
One-off CLI work on a store (`info`, `migrate`, building a separate static database) runs on the `haiku-rag` service; `skills/soliplex-template/scripts/rag_db.py` drives it for the static-database case.
...
- `backend/constraints.txt` pins `soliplex >= 0.82, < 0.83`. Bumping it is a backend rebuild. (The opt-in TUI pins the same package separately, in `tui/constraints.txt`.)
- The backend migrates `soliplex_agui` / `soliplex_authz` to the pinned release's Alembic head itself, on startup (each app role owns its schema, so `installation.yaml` sets no `migration_dburi` / `migration_policy`). The exception: databases created by soliplex <= 0.81 carry no `alembic_version` stamp, and 0.82+ refuses to start on them (`UnstampedDatabase`) until soliplex's one-time `scripts/bootstrap_alembic_version.py` has stamped them. `uv run scripts/migrate_soliplex_dbs.py --check` reports what a rebuilt backend image would demand; with `backend` stopped, the same command minus `--check` stamps and upgrades both. The ingester's `soliplex_ingester` queue is not Alembic-managed: the ingester migrates it in place on open.
- The `haiku-ingester` image tag `ghcr.io/ggozad/haiku.rag-slim:0.84.0` appears in both `docker-compose.yml` (once, as the `&haiku_rag_image` anchor the one-shot `haiku-rag` service aliases) and `haiku.rag/Dockerfile`, and the two must agree. It is not a free choice: the pinned `soliplex` release constrains `haiku.rag-slim` as a dependency, and the ingester writes the LanceDB the backend reads — check that constraint before changing the tag. Most bumps need nothing more, but a release that changed the LanceDB layout (haiku.rag ships a store upgrade only for those, and checks a store against that set rather than against the release number) leaves every store under `rag/db/` needing a one-off migration, without which the ingester crash-loops and reads fail. Check with `uv run scripts/migrate_rag_dbs.py --check`, then apply with the same command minus `--check` and `haiku-ingester` stopped.
...
- That export is why one-off CLI calls go through the **`haiku-rag`** service, whose entrypoint does the same thing. `docker compose run` *replaces* a service's command, so running the CLI on `haiku-ingester` loses the export and the config fails to load (`MissingEnvVarError`) before the subcommand starts — the queue `dburi` interpolates the same variable, and haiku.rag expands the whole file up front.
