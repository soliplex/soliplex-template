...
and parses the YAML output. Subcommands expose individual slices of that
config; today there is just ``room_ids`` (the structure leaves room for more).

``room_ids`` prints the id of every room the installation actually loads, one
per line. It is driven by the config's resolved ``room_paths`` -- so unlike a
``rooms/*`` glob it honors installations that limit their room set or point
``room_paths`` at shared directories. ``room_paths`` come back as the backend
container's absolute paths; each is mapped back to the host through the
backend's ``<host-environment> -> <installation>`` bind mount, and the
``id:`` of every ``room_config.yaml`` found beneath it (a directory may hold a
single room or a tree of them) is read on the host.

    python3 soliplex_config.py room_ids --project-dir /path/to/stack

Rooms whose resolved path lies outside ``--installation`` (an exotic shared
mount) cannot be mapped to a host file; they are reported on stderr and
...
_WIDE_COLUMNS = "10000"
_ROOM_ID_RE = re.compile(r"""^id:\s*["']?([^"'\s]+)["']?\s*$""")
...
def read_room_id(text: str) -> str | None:
    """The ``id:`` of a room_config.yaml, or None if it has no id line."""
    for line in text.splitlines():
        match = _ROOM_ID_RE.match(line)
        if match:
...
def resolve_rooms(
    ...
    """Map each loaded room ``id`` to its host room_config.yaml.

    Returns ``(id -> path, unmapped_container_paths)``. Room id conflicts
    resolve first-past-the-post in ``room_paths`` order, matching soliplex.
    """
    ...
    for container_path in config["room_paths"]:
        ...
        for cfg in find_room_configs(host_dir):
            room_id = read_room_id(cfg.read_text())
            if room_id and room_id not in rooms:
                rooms[room_id] = cfg
    return rooms, unmapped


def do_room_ids(args: argparse.Namespace) -> int:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rooms = sub.add_parser(
        "room_ids", help="print the id of every loaded room, one per line"
    )
    _add_common(rooms)
    rooms.set_defaults(func=do_room_ids)
